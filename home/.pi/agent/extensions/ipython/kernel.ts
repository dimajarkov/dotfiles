...
export interface ExecuteOptions {
	...
	/** Receives complete display_data, execute_result, and traceback text in IOPub order. */
...
export interface ExecuteResult {
	stdout: string;
	stderr: string;
	/** Text emitted through IPython display_data. */
	display?: string;
...
function makeConnection(): { info: ConnectionInfo; path: string; tempDir: string } {
	...
	const tempDir = mkdtempSync(join(tmpdir(), "pi-ipython-kernel-"));
...
function installSignalHandlersOnce(): void {
	// Pi owns process signals. The extension disposes its session kernel from
	// session_shutdown so kernel code must not install competing signal handlers.
}
...
export class KernelManager {
	...
	private handleExecutionMessage(incoming: JupyterMessage): void {
		...
		if (t === "stream") {
			...
			const text = c.data["text/plain"];
			if (text) {
				execution.opts.onRichOutput?.(text, "result");
			...
			for (const mimeType of ["image/png", "image/jpeg", "image/gif", "image/webp"] as const) {
				...
				if (encoded.length > MAX_ATTACHMENT_DATA_CHARS) {
					execution.stderr += `${execution.stderr ? "\n" : ""}attachment dropped: exceeds ${MAX_ATTACHMENT_DATA_CHARS} base64 chars`;
					execution.status = "error";
				} else {
					execution.attachments.push({ mimeType, data: encoded });
				}
			}
			const displayText = data["text/plain"];
			if (typeof displayText === "string" && displayText.length > 0) {
				execution.opts.onRichOutput?.(displayText, "display");
				const separator = execution.display ? "\n" : "";
				const remaining = execution.maxChars - execution.display.length - separator.length;
				if (remaining > 0) execution.display += separator + displayText.slice(0, remaining);
				if (displayText.length > remaining) execution.displayTruncated = true;
			}
			...
			execution.opts.onRichOutput?.(c.traceback.join("\n"), "error");
			execution.error = c;
			execution.status = "error";
		} else if (t === "status") {
	...
	private resolveExecution(execution: ActiveExecution, options: { clearActive: boolean }): void {
		...
		if (!execution.settled) {
			...
			let display = execution.display || undefined;
			...
			if (execution.displayTruncated && display !== undefined) {
				display += `\n[... output truncated at ${execution.maxChars} chars ...]`;
			}
