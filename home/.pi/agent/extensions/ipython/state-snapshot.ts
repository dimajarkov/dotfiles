...
//
// Snapshotting is best-effort: each top-level name is checked independently so
// an unpicklable object is skipped, then admitted values are serialized as one
// object graph so aliases and shared references survive.
...
def _prime_agent_snapshot_state():
    ...
    for name in _b.list(ns.keys()):
        ...
        if _b.len(blob) > ${maxBytes} or total + _b.len(blob) > ${maxBytes}:
            ...
            continue
        # Keep the values in one object graph so aliases and shared references
        # survive. The per-value dump above is only an admission and size check.
        payload[name] = value
        total += _b.len(blob)

    directory = os.path.dirname(${pyStr(outPath)})
    ...
    try:
        ...
        with os.fdopen(fd, "wb") as fh:
            dill.dump({"__pi_ipython_snapshot_format__": 2, "values": payload}, fh)
        actual_bytes = os.path.getsize(tmp)
        if actual_bytes > ${maxBytes}:
            os.remove(tmp)
            _b.print(${pyStr(RESULT_MARKER)} + json.dumps({"error": "serialized payload exceeds snapshot size cap"}))
            return
    ...
    try:
        manifest_fd = os.open(${pyStr(manifestPath)}, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
        with os.fdopen(manifest_fd, "w") as fh:
 ...
 * reviving the saved object graph, then prints a single marker line with the result.
...
def _prime_agent_restore_state():
    ...
    if not os.path.exists(${pyStr(inPath)}):
        ...
        return
    try:
        os.chmod(os.path.dirname(${pyStr(inPath)}), 0o700)
        os.chmod(${pyStr(inPath)}, 0o600)
    except _b.Exception as _err:
        _b.print(${pyStr(RESULT_MARKER)} + json.dumps({"restored": [], "failed": [], "error": "permission hardening failed: " + _b.str(_err)}))
        return
    try:
        import dill
    except _b.Exception as _err:
    ...
    # dill normally rebuilds __main__ functions with a private globals dict.
    # Override its pinned function factory only while unpickling so every user
    # function is created against the live IPython namespace from the outset.
    # This preserves identities, immutable container subclasses, and arbitrary
    # cyclic graphs without a lossy post-load graph rewrite.
    def load_with_live_globals(loader):
        try:
            import dill._dill as _dill_impl
            original_create_function = _dill_impl._create_function
        except _b.Exception:
            return loader()

        def create_function(fcode, fglobals, fname=None, fdefaults=None, fclosure=None, fdict=None, fkwdefaults=None):
            if _b.isinstance(fglobals, _b.dict) and fglobals.get("__name__") in ("__main__", None):
                fglobals = ns
            return original_create_function(fcode, fglobals, fname, fdefaults, fclosure, fdict, fkwdefaults)

        _dill_impl._create_function = create_function
        try:
            return loader()
        finally:
            _dill_impl._create_function = original_create_function

    try:
        with _b.open(${pyStr(inPath)}, "rb") as fh:
            loaded = load_with_live_globals(lambda: dill.load(fh))
    except _b.Exception as _err:
        _b.print(${pyStr(RESULT_MARKER)} + json.dumps({"restored": [], "failed": [], "error": "load failed: " + _b.str(_err)}))
        return
    if not _b.isinstance(loaded, _b.dict):
        _b.print(${pyStr(RESULT_MARKER)} + json.dumps({"restored": [], "failed": [], "error": "corrupt snapshot: not a dict"}))
        return

    # Version 1 stored one dill blob per name. Version 2 wraps one shared graph
    # in an explicit envelope so a legitimate bytes value is never misread as
    # a legacy pickle.
    is_shared_graph = (
        loaded.get("__pi_ipython_snapshot_format__") == 2
        and _b.isinstance(loaded.get("values"), _b.dict)
    )
    payload = loaded["values"] if is_shared_graph else loaded

    restored = []
    failed = []
    for name, value in payload.items():
        try:
            if not is_shared_graph:
                value = load_with_live_globals(lambda: dill.loads(value))
            ns[name] = value
