...
const records = () =>
  registry().filter((record) => roots.has(record.rootId) && record.semanticName);
function refreshOwnedPanes() {
  for (const record of records())
    if (record.paneId) ownedPanes.set(record.paneId, record.sessionPath);
  // Scope allocation may persist its pane before the child's startup record is saved.
  for (const record of registry())
    if (roots.has(record.rootId)) {
      for (const pane of record.ownedPaneIds ?? [])
        if (!ownedPanes.has(pane)) ownedPanes.set(pane, undefined);
    }
...
function noTestFocus() {
  ...
  assert.ok(
    panes.every((pane) => !ownedPanes.has(pane.pane_id) || !pane.focused),
    "Test surfaces must not steal human focus",
  );
}
function getPane(id) {
  try {
    return herdr("pane", "get", id).pane;
  } catch (error) {
    if (error.herdrCode === "pane_not_found") return undefined;
    throw error;
  }
}
const find = (root, name) =>
  records().find((record) => record.rootId === root.id && record.semanticName === name);
const call = (arguments_, expectError = false) => ({
  name: "subagent",
  arguments: arguments_,
  expectError,
});
const gate = (key, participant, blocked = false) => ({
  name: "scope_gate",
  arguments: { key, participant, blocked },
});
const plan = (calls, result = "CHILD_FINISHED") =>
  JSON.stringify({ scopeFixture: true, calls, result });
const spawn = (name, agent, workScope, calls = []) =>
  call({ action: "spawn", name, agent, ...(workScope ? { workScope } : {}), task: plan(calls) });
const release = (key) => writeFileSync(join(directory, "gates", `${key}.release`), "release\n");
const ready = (key, participant) =>
  until(() => {
...
async function command(root, calls) {
  ...
  await until(
    () => {
      const output = entries(root.session)
        .slice(start)
        ...
        .map((part) => part.text);
      assert.ok(
        !output.includes("FIXTURE_UNEXPECTED_TOOL_OUTCOME"),
        "Unexpected tool result in root session",
      );
      return output.includes(marker);
    },
    `root command ${calls.map((c) => c.arguments.action).join(",")}`,
    60_000,
  );
  noTestFocus();
}
async function root(label) {
  ...
  const session =
    started.agent.agent_session?.value ??
    (await until(
      () => herdr("agent", "get", name).agent.agent_session?.value,
      "root session reporting",
    ));
...
const verifyAllRuntimes = () =>
  verifyRuntime([...ownedPanes.values(), ...records().map((record) => record.sessionPath)]);
...
symlinkSync(
  join(here, "test-fixtures", "scope-provider.ts"),
  join(directory, "extensions", "scope-provider.ts"),
);
for (const role of ["worker", "scout", "researcher", "planner", "reviewer"]) {
  const thinking = role === "worker" ? "high" : "medium";
  writeFileSync(
    join(directory, "agents", `${role}.md`),
...
async function reassignedCase(a) {
  await command(a, [
    spawn("reassigned", "reviewer", "reassigned", [gate("reassigned", "owner", true)]),
  ]);
  ...
  assert.ok(
    !existsSync(reassigned.completionMarkerPath),
    "The interrupted child has no completion proof",
  );
  ...
  try {
    ...
    const replacement = herdr(
      "agent",
      "start",
      reassigned.herdrName,
      "--kind",
      "pi",
      "--pane",
      reassigned.paneId,
    ...
    const session =
      replacement.agent.agent_session?.value ??
      (await until(
        () => getPane(reassigned.paneId).agent_session?.value,
        "replacement session reporting",
      ));
  ...
  await until(
    () => getPane(a.pane).agent_session?.value === a.session,
    "parent saved-session restart",
  );
  await until(() => find(a, "reassigned").state === "stale", "reassigned session becomes stale");
  const staleSnapshot = find(a, "reassigned");
  await command(a, [
    call({ action: "message", name: "reassigned", message: plan([], "WRONG_RECIPIENT") }, true),
  ]);
  assert.deepEqual(
    find(a, "reassigned"),
    staleSnapshot,
    "Rejected follow-up preserves the prior registry record",
  );
  ...
  assert.equal(
    find(a, "reassigned-peer").paneId,
    undefined,
    "A reassigned anchor is rejected before allocation",
  );
  await command(a, [call({ action: "cancel", name: "reassigned" })]);
  assert.ok(
    getPane(reassigned.paneId),
    "Cancelling a stale record must preserve the unrelated current session",
  );
  assert.equal(find(a, "reassigned").surfaceState, "released");
  await command(a, [
    spawn("after-reassign", "reviewer", "reassigned", [gate("after-reassign", "peer")]),
  ]);
  await ready("after-reassign", "peer");
  assert.equal(
    find(a, "after-reassign").tabId,
    a.tab,
    "A new scope stage splits the current master pane",
  );
  ...
  assert.ok(
    getPane(reassigned.paneId),
    "Released ownership must not be reclaimed after the replacement exits",
  );
  await command(a, [
    call({ action: "message", name: "reassigned", message: plan([], "RESTORED_ORIGINAL_SESSION") }),
  ]);
  await completed(a, "reassigned", 2);
  const restored = find(a, "reassigned");
  assert.equal(
    restored.sessionPath,
    reassigned.sessionPath,
    "Released children reactivate their original saved session",
  );
  ...
  assert.ok(
    getPane(reassigned.paneId),
    "Saved-session reactivation preserves the released user shell",
  );
}
function checkGeometry(paneId, count, workScope) {
  ...
  assert.ok(
    relatedPanes.every(
      (pane) => pane.rect.x === layout.area.x && pane.rect.width === layout.area.width,
    ),
    `${count} related panes must use horizontal splits, preserving the full tab width`,
  );
  assert.ok(
    layout.splits.every((split) => split.direction === "down"),
    "Every split must stack panes top-to-bottom",
  );
  const heights = relatedPanes.map((pane) => pane.rect.height);
  assert.ok(
    heights.every((height) => height > 0),
    `${count} related panes retain visible rows`,
  );
  ...
  if (count !== 3) {
    assert.ok(
      Math.max(...heights) <= 2 * Math.min(...heights) + 1,
      `${count} related panes are balanced`,
    );
  }
}
async function blockedCancellationCase(a) {
  await until(
    () =>
      find(a, "different-scope")?.state === "blocked" &&
      herdr("agent", "get", find(a, "different-scope").herdrName).agent.agent_status === "blocked",
    "blocked fixture and monitor state",
  );
  const before = find(a, "different-scope");
  const sessionStart = entries(a.session).length;
  await command(a, [call({ action: "cancel", name: "different-scope" }, true)]);
  const errors = entries(a.session)
    .slice(sessionStart)
    .filter((entry) => entry.message?.role === "toolResult" && entry.message.isError);
  assert.match(JSON.stringify(errors), /agent_blocked/);
  const after = find(a, "different-scope");
  for (const key of ["state", "generation", "sessionPath", "paneId", "result", "deliveredAt"]) {
    assert.equal(after[key], before[key], `Rejected cancellation preserves ${key}`);
  }
  assert.ok(getPane(after.paneId), "No fallback interrupt or cleanup closes the blocked child");
  assert.equal(herdr("agent", "get", after.herdrName).agent.agent_status, "blocked");
  // Only the fixture owner releases this gate, simulating the user resolving
  // blocked input. The failed cancellation must not release or abort it.
  release("other");
  await completed(a, "different-scope");
  const completedResult = find(a, "different-scope").result;
  await command(a, [call({ action: "cancel", name: "different-scope" })]);
  assert.equal(find(a, "different-scope").state, "completed");
  assert.equal(find(a, "different-scope").result, completedResult);
  assert.equal(find(a, "different-scope").surfaceState, "closed");
}

async function runCases() {
  if (process.env.SCOPE_CASES === "blocked") {
    const a = await root("blocked");
    await command(a, [
      spawn("different-scope", "reviewer", "different-scope", [gate("other", "other", true)]),
    ]);
    await ready("other", "other");
    await blockedCancellationCase(a);
    verifyAllRuntimes();
    console.log(
      "PASS: blocked cancellation fails without interrupting, then preserves the released conclusion",
    );
    return;
  }
  ...
  await command(a, [
    call({ action: "spawn", agent: "scout", name: "missing-scope", task: "unused" }, true),
  ]);
  ...
  await command(a, [
    spawn("worker-a", "worker", "shared-scope", [
      ...
      gate("start", "a"),
      spawn("research-a", "researcher", undefined, [gate("research", "a")]),
      gate("density-a", "a"),
      spawn("density-7", "scout", undefined, [gate("density-7", "7")]),
      gate("worker-a", "a"),
    ]),
    spawn("worker-b", "worker", "shared-scope", [
      gate("start", "b"),
      spawn("scout-b", "scout", undefined, [gate("scout", "b")]),
      gate("density-b", "b"),
      spawn("density-8", "researcher", undefined, [gate("density-8", "8")]),
      gate("worker-b", "b"),
    ]),
  ]);
  ...
  const [researchRuntime, scoutRuntime] = await Promise.all([
    ready("research", "a"),
    ready("scout", "b"),
    ready("density-a", "a"),
    ready("density-b", "b"),
  ...
  for (const count of [5, 6, 7, 8]) {
    if (count <= 6) {
      await command(a, [
        spawn(`density-${count}`, "reviewer", "shared-scope", [
          gate(`density-${count}`, String(count)),
        ]),
      ]);
  ...
  await ready("other", "other");
  assert.equal(
    find(a, "different-scope").tabId,
    scopeTab,
    "Different scopes still use the master tab",
  );
  const b = await root("b");
  await command(b, [
    spawn("unrelated", "reviewer", "shared-scope", [gate("unrelated", "unrelated")]),
  ]);
  await ready("unrelated", "unrelated");
  assert.equal(
    find(b, "unrelated").tabId,
    b.tab,
    "A separate conversation uses its own master tab",
  );
  assert.notEqual(b.tab, scopeTab, "Separate conversations remain isolated");
  await command(a, [
    spawn("no-delegation", "scout", "permission-test", [
      { ...spawn("illegal-child", "scout"), expectError: true },
    ]),
  ]);
  ...
  await command(a, [
    call({ action: "message", name: "worker-a", message: plan([gate("followup", "a")]) }),
  ]);
  ...
  await command(a, [
    call({ action: "message", name: "worker-a", message: plan([gate("sequential", "a")]) }),
  ]);
  ...
  assert.equal(
    sequential.tabId,
    scopeTab,
    "No owned anchor means splitting the current master pane, not human panes",
  );
  ...
  assert.equal(
    find(a, "moved-peer").paneId,
    undefined,
    "Moved-anchor rejection happens before allocation",
  );
  ...
  assert.equal(
    find(a, "after-move").tabId,
    scopeTab,
    "A new stage reuses the master tab after the moved agent finishes",
  );
  release("post-move");
  ...
  for (const record of records()) {
    if (record.paneId) {
      ...
      assert.ok(
        record.surfaceState === "closed" ||
          (safelyReleasedMovedPane && record.surfaceState === "released"),
      );
  ...
  writeFileSync(
    ...
    JSON.stringify({ roots: [a, b], records: records() }, null, 2),
  );
  verifyAllRuntimes();
  console.log(
    "PASS: concurrent cross-parent placement, inheritance, role permissions, lineage isolation, usable geometry, sibling/user-pane preservation, saved-session reactivation, sequential stages, moved-anchor safe rejection, blocked cancellation and reassigned-occupant preservation",
  );
}
try {
  await runCases();
  ...
  for (const pane of ownedPanes.keys()) {
    try {
      writeFileSync(
        join(directory, `failure-${pane.replaceAll(":", "-")}.txt`),
        execFileSync("herdr", ["pane", "read", pane, "--source", "visible"], {
          encoding: "utf8",
          timeout: 5_000,
          stdio: "pipe",
        }),
      ...
      /* A completed test surface may already have closed. */
    }
  }
  throw error;
} finally {
