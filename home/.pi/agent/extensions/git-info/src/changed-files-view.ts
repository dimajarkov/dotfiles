...
const run = (cwd: string, args: string[]) => runCommand("git", args, cwd, 10_000);
...
const loadFile = Effect.fn("git-info.loadFile")(function* (
  ...
  const diffArguments = useNoIndex
    ...
    : ["diff", "--no-ext-diff", "--no-color", "--unified=3", "HEAD", "--", changedPath.path];
  ...
  return {
    ...stats,
    diff: diff.length === 1 && diff[0] === "" ? ["No textual diff available."] : diff,
    name: cleanDisplayPath(basename(changedPath.path)),
    path: cleanDisplayPath(changedPath.path),
  } satisfies ChangedFile;
});

export const loadChangedFiles = Effect.fn("git-info.loadChangedFiles")(function* (cwd: string) {
  ...
  const [statusResult, headResult] = yield* Effect.all(
    [
      run(repoRoot, ["status", "--porcelain=v1", "-z", "--untracked-files=all"]),
...
export async function showChangedFiles(ctx: ExtensionContext, files: ChangedFile[]) {
  ...
  await ctx.ui.custom<void>(
    (tui, theme, _keybindings, done) => {
      ...
      function moveDiff(amount: number) {
        const maxOffset = Math.max(0, files[selectedIndex]!.diff.length - bodyHeight());
      ...
      function handleInput(data: string) {
        ...
        if (matchesKey(data, Key.end) || data === "G") {
          diffOffset = Math.max(0, files[selectedIndex]!.diff.length - bodyHeight());
          tui.requestRender();
        }
      }
      ...
      function render(width: number) {
        ...
        for (let row = 0; row < height; row += 1) {
          ...
          if (file) {
            ...
            if (row % 2 === 0) {
              ...
              const gap = " ".repeat(
                Math.max(
                  1,
                  sidebarWidth - visibleWidth(marker) - visibleWidth(name) - visibleWidth(stats),
                ),
              );
            ...
            if (isSelected) {
              sidebar = theme.bg(focus === "files" ? "selectedBg" : "customMessageBg", sidebar);
            }
          } else {
          ...
          const diff = padToWidth(diffLine === undefined ? "" : styleDiffLine(diffLine), diffWidth);
          const separator = theme.fg(focus === "diff" ? "borderAccent" : "borderMuted", "│");
          lines.push(
