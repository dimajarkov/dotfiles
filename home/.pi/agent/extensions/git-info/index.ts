...
export default function gitInfo(pi: ExtensionAPI) {
  ...
  const run = (command: string, args: string[], ctx: ExtensionContext, timeout: number) =>
  ...
  const refreshEffect = (
    ...
    Effect.suspend(() => {
      ...
      return Effect.gen(function* () {
        const repo = yield* run("git", ["rev-parse", "--is-inside-work-tree"], ctx, GIT_TIMEOUT_MS);
        ...
        const [branchResult, headResult, statusResult] = yield* Effect.all(
          [
            ...
            run("git", ["status", "--porcelain=v1", "--untracked-files=all"], ctx, GIT_TIMEOUT_MS),
          ],
  ...
  const poll = () =>
    Effect.suspend(() => (currentContext ? refreshIfIdle(currentContext) : Effect.void)).pipe(
  ...
  const forkBackground = (effect: Effect.Effect<void, never, CommandRunner>) =>
    getRuntime().runFork(effect.pipe(Effect.catchDefect(reportBackgroundDefect)));
