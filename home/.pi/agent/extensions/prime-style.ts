...
function withBackground(theme: AppTheme, background: Background, line: string): string {
  // The legacy Pi editor uses a full SGR reset for its cursor. Reapply both
  // surface colors after resets, including when the terminal itself is dark.
  return line
    .split("\x1b[0m")
    .map((segment) => theme.bg(background, theme.fg("text", segment)))
    .join("\x1b[0m");
}
...
class PrimeEditor extends CustomEditor {
  ...
  override render(width: number): string[] {
    ...
    const bottomBorderIndex = raw.findIndex(
      (line, index) => index > 0 && isEditorBorder(line, width),
    );
...
export default function (pi: ExtensionAPI) {
  pi.on("session_start", (_event, ctx) => {
    // This extension owns the editor layout, not the palette. In particular,
    // do not overwrite the appearance selected by mac-system-theme.
