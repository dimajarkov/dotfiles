...
def cli() -> None:
    """Apply one exact edit from a shell cell."""
    import argparse
    import asyncio

    parser = argparse.ArgumentParser(description="Replace one exact string in a file")
    parser.add_argument("--path", required=True)
    parser.add_argument("--old-str", required=True)
    parser.add_argument("--new-str", required=True)
    args = parser.parse_args()
    print(asyncio.run(run(args.path, args.old_str, args.new_str)))
