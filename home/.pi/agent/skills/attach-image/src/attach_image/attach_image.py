...
async def run(*paths: str) -> str:
    ...
    if os.environ.get("PI_MODEL_SUPPORTS_IMAGES") == "0":
        raise RuntimeError(
            "The current Pi model does not support vision. "
            "Switch to a vision-capable model before loading images into context."
        )
...
def cli() -> None:
    """Load images from a shell cell using the same bridge as the Python API."""
    import argparse
    import asyncio

    parser = argparse.ArgumentParser(description="Load images into the Pi model context")
    parser.add_argument("paths", nargs="+", help="PNG, JPEG, GIF, or WebP paths")
    args = parser.parse_args()
    print(asyncio.run(run(*args.paths)))
